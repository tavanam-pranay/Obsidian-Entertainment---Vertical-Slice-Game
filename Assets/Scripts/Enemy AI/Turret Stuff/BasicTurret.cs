...
{
    public EnemyFrequencyHandler frequencyHandler;
    ...
    {
        if (frequencyHandler.currentState != EnemyStates.Ignorant)
        {
            ...
            {
                frequencyHandler.currentState = EnemyStates.Hostile;
    ...
    private void OnTriggerExit2D(Collider2D other) // Ensures the turret stops shooting when player leaves.
    {
        if (other.CompareTag("Player"))
        {
            ...
            if(frequencyHandler.currentState != EnemyStates.Ignorant) frequencyHandler.currentState = EnemyStates.Scanning;
        }
    }
}
